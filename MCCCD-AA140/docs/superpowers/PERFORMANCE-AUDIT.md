...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/AudioMixer.svelte`, `src/pages/DisplayRouting.svelte`* — *risk: medium* — *expected: ~23 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 21 mixer-state subscriptions (audioOutputSelectFb, micLav/Handheld/Ceiling1-3 trim/lineOut/connected, sceneRecallFb, audioLinkCeilings12Fb) moved to `initMixerStateSubscriptions()`; 2 routing subscriptions (routingModeFb, autoRouteEnableFb) moved to `initRoutingStateSubscriptions()`. camTrackingModeFb left global (Cameras.svelte is user WIP — not modifiable). `initSignals()` reduced from 35 → 12 active subscriptions at boot. Note: micCeiling3 signals (mute/trim/lineOut/connected) are included in mixer init but are actually unused by any component — legitimate but harmless. Bundle size: index-*.js 76,730 B (branch now includes new AppFooter + routing v2 features added after prior loop; direct comparison to 77,221 B baseline is not apples-to-apples).*
...
| 2026-05-04 | Loop final summary (prior loop) | f5201e1 | All HIGH addressed (at the time), stop conditions met |
| 2026-06-14 | H4-followup per-page state signal gating | pending iter-10 | initSignals() 35→12 subscriptions at boot; 21 mixer-only + 2 routing-only gated per-page |
