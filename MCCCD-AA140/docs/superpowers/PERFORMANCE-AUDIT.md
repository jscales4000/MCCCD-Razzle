...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/AudioMixer.svelte`, `src/pages/DisplayRouting.svelte`* — *risk: medium* — *expected: ~14 fewer subscriptions at boot* — **DONE iter-10** — *actual: gated 12 ceiling-mic signals (mute/trim/lineOut/connected × 3 channels) + sceneRecallFb + audioLinkCeilings12Fb → AudioMixer; routingModeFb + autoRouteEnableFb → DisplayRouting. Boot subscriptions reduced from ~27 (post-H4) to ~13 common signals. JS +604 B for the new init/teardown function bodies — expected trade-off. Cameras.svelte (WIP) protected: camTrackingModeFb left in initSignals().*
...
| 2026-06-28 | H4-followup gate low-freq state signals | (this iter-10) | JS +604 B for gating bodies; -14 boot crcomlib subscriptions (27→13 common) |
