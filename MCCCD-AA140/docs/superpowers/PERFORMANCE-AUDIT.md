...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial)** — *actual: 27 subscriptions moved out of initSignals() (6 → DisplayRouting via initDisplayRoutingSubscriptions(), 21 → AudioMixer via initAudioMixerSubscriptions()). initSignals() now wires only 12 always-on subscriptions. camTrackingModeFb left always-on (Cameras.svelte is user WIP). Bundle delta: negligible (removed inline callbacks ≈ added wrapper fns). index-*.js 76,750 B / index-*.css 29,728 B (baseline includes feature commits since iter-9).*
...
| 2026-06-30 | H4-followup per-page signal gating | 6f0d2c9 | 27 subscriptions moved to per-page lazy init; initSignals() → 12 always-on; bundle delta negligible |
