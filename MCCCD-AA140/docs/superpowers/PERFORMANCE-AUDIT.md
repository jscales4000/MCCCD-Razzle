...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`, `src/pages/AudioMixer.svelte`* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 21 subscriptions removed from always-on initSignals() (2 routing + 19 mixer-exclusive). camTrackingModeFb left always-on — gating it requires Cameras.svelte (protected WIP). micLavMuteFb, micHandheldMuteFb, progAudioLevelFb left always-on — consumed by Aa140Footer on all three active pages. JS -503 B net (vs 2026-05-04 final 77,221 B); note: post-loop feature commits added CSS/JS so the net delta mixes feature additions with the perf reduction. Subscription count at idle: 14 always-on → 14+2 routing when DisplayRouting mounted → 14+19+5 mixer+levels when AudioMixer mounted.*
...
| 2026-05-04 | Loop final summary | f5201e1 | All HIGH addressed, stop conditions met (original loop end) |
| 2026-06-19 | H4-followup per-page signal gating | pending iter-10 | 21 subscriptions removed from always-on; JS -503 B net (mixed with 3-week feature delta); camTrackingModeFb left always-on (Cameras.svelte protected) |
