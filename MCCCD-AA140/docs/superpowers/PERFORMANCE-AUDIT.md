...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (scoped)** — *actual: extracted 2 routing signals (`routingModeFb` analog + `autoRouteEnableFb` digital) into `initRoutingSignals()` / `teardownRoutingSignals()`. DisplayRouting wires them via onMount/onDestroy. `camTrackingModeFb` skipped (can't modify Cameras.svelte). AudioMixer ceiling-signal gating skipped — those 15 subscriptions fire only on user action (not continuously) and the ceiling3 stores are pre-declared infrastructure for a not-yet-wired third mic array. index-*.js −664 B (76,557 B) as 2 subscription callbacks moved from the always-on init path into the DisplayRouting lazy chunk.*
...
| 2026-06-16 | H4-followup routing signal gate (scoped) | pending iter-10 | index-*.js −664 B (76,557 B); `routingModeFb` + `autoRouteEnableFb` now gated to DisplayRouting onMount/onDestroy |
