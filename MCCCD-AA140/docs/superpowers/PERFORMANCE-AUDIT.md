...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/AudioMixer.svelte`, `src/pages/DisplayRouting.svelte`* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 22 subscriptions moved out of `initSignals()` (20 AudioMixer-only: ceiling mutes/trims/lineOuts/connected, sceneRecallFb, audioLinkCeilings12Fb, micLav/HandheldTrimFb/LineOutFb/Connected; 2 DisplayRouting-only: routingModeFb, autoRouteEnableFb). `initSignals()` reduced 36→14 subs at boot. `camTrackingModeFb` kept in initSignals() — Cameras.svelte is user WIP. index JS +337 B for the new management functions (expected; this is a runtime not a bundle item). micLavMuteFb/micHandheldMuteFb and progAudioLevelFb kept in initSignals() — also consumed by Home.svelte.*
...
| 2026-05-23 | H4-followup per-page state signal gating | 9a332db | 22 subs out of initSignals() (36→14 at boot); index JS +337 B (nominal, runtime item not bundle item) |
