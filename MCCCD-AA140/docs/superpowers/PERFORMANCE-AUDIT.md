...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial: 16 of 17 signals gated)** — *actual: 16 subscriptions moved out of initSignals() (2→DisplayRouting, 14→AudioMixer). camTrackingModeFb stays global — Cameras.svelte is user WIP. initSignals() shrinks from ~37 to ~21 subscriptions at boot. Bundle +301 B index.js / +115 B index.css for the gate functions — expected and acceptable for a runtime improvement.*
...
| 2026-05-04 | Loop final summary | f5201e1 | All HIGH addressed (prior loop), stop conditions met |
| 2026-05-14 | H4-followup per-page signal gating (partial) | pending iter-10 | 16 subs moved: 2→DisplayRouting, 14→AudioMixer; initSignals() 37→21; index.js +301 B, index.css +115 B (gate-fn overhead) |
