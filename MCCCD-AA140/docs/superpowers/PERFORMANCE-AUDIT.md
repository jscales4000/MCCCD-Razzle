...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 23 callbacks removed from crcomlib registry at idle (16 AudioMixer-only state signals gated via initAudioMixerStateSubscriptions(), 2 routing signals via initRoutingSubscriptions(), 5 dead ceiling-3 subs removed including level sub from H4's mic gate). Cameras.svelte is protected so camTrackingModeFb remains always-on. JS index -153 B (-0.2%) — marginal as predicted, runtime win is the registry cleanup.*
...
| 2026-05-04 | Loop final summary | f5201e1 | All HIGH addressed (at the time), stop conditions met |
| 2026-06-01 | H4-followup gate low-freq state signals | pending iter-10 | 23 callbacks removed from crcomlib registry at idle; JS −153 B (−0.2%); camTrackingModeFb stays always-on (Cameras.svelte protected) |
