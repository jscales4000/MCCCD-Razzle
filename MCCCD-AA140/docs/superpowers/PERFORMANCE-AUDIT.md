...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/AudioMixer.svelte`, `src/pages/DisplayRouting.svelte`* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (scoped)** — *actual: 16 subscriptions gated (14 AudioMixer-only: ceiling mic mute/trim/lineOut/connected × 3, sceneRecallFb, audioLinkCeilings12Fb; 2 DisplayRouting-only: routingModeFb, autoRouteEnableFb). camTrackingModeFb left in initSignals() — Cameras.svelte is user WIP. index.js +523 B for the gating functions (expected); runtime: 16 fewer crcomlib callback registrations at boot.*
...
| 2026-05-12 | H4-followup gate per-page state signals | pending iter-10 | 16 crcomlib subscriptions gated (14 AudioMixer + 2 DisplayRouting); index.js +523 B for gating functions; camTrackingModeFb left always-on (Cameras.svelte is WIP) |
