...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (scoped)** — *actual: 16 subscriptions gated (2 routing + 14 mixer state). Bundle +390 B raw (gating functions live in main bundle — expected trade-off). camTrackingModeFb stayed always-on (Cameras.svelte is user WIP). micLav/Handheld mutes stayed in initSignals (Aa140Footer uses them on Home + DisplayRouting).*
...
| 2026-06-18 | H4-followup routing+mixer signal gating | ffcb92f | 16 subscriptions gated; bundle +390 B raw (gating logic overhead); 16 fewer idle crcomlib callbacks |
