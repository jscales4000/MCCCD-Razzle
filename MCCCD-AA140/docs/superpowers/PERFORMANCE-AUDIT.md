...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: initSignals() boot subscriptions 37 → 15 (-22); micLevelSubscriptions 5 → 4 (ceiling3 dead); 4 dead ceiling3 subscriptions eliminated; AudioMixer JS -490 B, index JS -324 B; ceiling3 was dead due to 4-mic redesign (Shure P300 = 4 channels)*
...
| 2026-05-04 | Loop final summary (iter-9) | f5201e1 | All HIGH (except H4-followup) addressed; loop paused |
| 2026-05-31 | H4-followup per-page gate mixer+routing subs | e4733b5 | initSignals 37→15 subs (-22); dead ceiling3 removed; AudioMixer JS -490 B, index JS -324 B |
