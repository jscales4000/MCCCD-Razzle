...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (scoped)** — *actual: −7 subscriptions removed from `initSignals()` at boot: 5 dead micCeiling3 signals (store + subscription + level-meter entry — hardware strip dropped per AudioMixer.svelte comment) + 2 routing signals now lazy via `initDisplayRoutingSignals()` / `teardownDisplayRoutingSignals()` wired in DisplayRouting.svelte onMount/onDestroy. index JS −470 B. camTrackingModeFb stays always-on — full gating requires Cameras.svelte (protected). AudioMixer-specific signals (trim/lineOut/sceneRecall/link) left in initSignals(); zero CPU win from gating them since they only fire on user action.*
...
| 2026-06-03 | H4-followup dead-signal purge + routing gate (scoped) | pending iter-10 | −5 dead micCeiling3 stores/subs; routingModeFb+autoRouteEnableFb now lazy via DisplayRouting onMount/onDestroy; index JS −470 B |
