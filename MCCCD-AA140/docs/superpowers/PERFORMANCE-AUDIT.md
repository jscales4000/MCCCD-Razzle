...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`, `src/pages/AudioMixer.svelte`* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial)** — *actual: gated 4 signals (routingModeFb + autoRouteEnableFb → DisplayRouting; sceneRecallFb + audioLinkCeilings12Fb → AudioMixer). camTrackingModeFb stays global (Cameras.svelte is user WIP). progAudioLevelFb stays global (also consumed by Home). index.js +269 B for gating logic — negligible. Runtime: 4 fewer always-on CrComLib subscriptions at idle.*
...
| 2026-05-11 | H4-followup per-page routing+mixer state gating | pending iter-10 | index.js +269 B (gating logic); 4 fewer always-on subscriptions; camTrackingModeFb stays global (Cameras WIP) |
