...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`, `src/pages/AudioMixer.svelte`* — *risk: medium* — *expected: ~4 fewer subscriptions at idle (partial — camTrackingModeFb left in initSignals because Cameras.svelte is user WIP)* — **DONE iter-10** — *actual: 4 subscriptions gated (routingModeFb + autoRouteEnableFb → DisplayRouting onMount/onDestroy; sceneRecallFb + audioLinkCeilings12Fb → AudioMixer onMount/onDestroy). Bundle +380 B total (scaffolding for 4 new lazy-init functions, same profile as H4). camTrackingModeFb stays global.*
...
| 2026-06-26 | H4-followup per-page routing+mixer state | b30ee7e | 4 subscriptions gated; bundle +380 B (scaffolding); camTrackingModeFb left global (Cameras.svelte WIP) |
