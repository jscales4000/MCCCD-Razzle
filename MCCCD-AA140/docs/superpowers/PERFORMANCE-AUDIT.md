...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial)** — *actual: gated routingModeFb + autoRouteEnableFb to DisplayRouting (initRoutingStateSubscriptions / teardownRoutingStateSubscriptions) and sceneRecallFb + audioLinkCeilings12Fb to AudioMixer (initMixerStateSubscriptions / teardownMixerStateSubscriptions). 4 fewer always-on crcomlib subscriptions at idle. camTrackingModeFb intentionally left in initSignals() — Cameras.svelte is user WIP (protected). index-*.js +103 B for the new gating functions.*
