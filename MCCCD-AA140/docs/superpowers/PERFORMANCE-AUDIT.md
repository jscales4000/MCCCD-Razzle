...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/AudioMixer.svelte`, `src/pages/DisplayRouting.svelte`* — *risk: medium* — *expected: ~22 fewer subscriptions at idle; minor memory* — **DONE iter-10 (partial — camTrackingModeFb left in common; Cameras.svelte WIP)** — *actual: 20 AudioMixer-specific signals (trim/lineOut/connected for all 5 mics + ceiling mutes + sceneRecallFb + audioLinkCeilings12Fb) gated via `initAudioMixerSubscriptions()`; 2 DisplayRouting signals (routingModeFb + autoRouteEnableFb) gated via `initRoutingSubscriptions()`. Main bundle +589 B JS / +119 B CSS for the new lazy init/teardown functions. crcomlib boot registry: 37 → 15 always-active subscriptions. camTrackingModeFb left in common (Cameras.svelte is user WIP).*
