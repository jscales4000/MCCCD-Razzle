...
- [~] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DEFERRED iter-10** — *camTrackingModeFb cannot be moved without touching Cameras.svelte (protected user WIP); partial impl (routing + mixer signals only) has medium refactor risk for zero runtime CPU gain; signal description itself says defer unless requested*
...
- [x] **M1. Move `unsubscribeSerial` / `publishSerial` exports out of `CrComLib.ts` (or annotate `/* @__PURE__ */`)** — *files: `src/lib/CrComLib.ts`* — *risk: low* — *expected: ~150 B JS (esbuild already tree-shakes, but verify)* — **DONE iter-10** — *actual: deleted `publishSerial`, `subscribeSerial`, `unsubscribeDigital`, `unsubscribeSerial` (all confirmed unused project-wide by grep). Bundle delta: JS -3 B (77,221 → 77,218) — Rollup was already tree-shaking these exports. Zero bundle impact confirmed; change is code quality / dead-code removal.*
...
| 2026-05-04 | Loop final summary | f5201e1 | All HIGH addressed, loop graduated |
| 2026-05-17 | H4-followup deferred | iter-10 | Cameras.svelte protected (WIP); zero runtime CPU gain; deferred |
| 2026-05-17 | M1 delete unused CrComLib serial exports | iter-10 | JS -3 B (77,221→77,218); Rollup already tree-shook; dead code removed |
