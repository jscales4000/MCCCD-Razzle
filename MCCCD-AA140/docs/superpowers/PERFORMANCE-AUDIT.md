...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: initSignals() subscriptions at boot: 39 → 9 (−30 idle subs). +9 gated to DisplayRouting (display2-4SourceFb, display1-4PowerFb, routingModeFb, autoRouteEnableFb). +18 gated to AudioMixer (audioOutputSelectFb, sceneRecallFb, audioLinkCeilings12Fb, ceiling1-2 mute/trim/lineOut/connected, lav/handheld trim/lineOut/connected). Also removed 5 dead micCeiling3* subscriptions (ceiling3 strip was dropped from AudioMixer in 2026-05-26 redesign; no consumer exists). progAudioLevelFb stays always-on because Aa140Footer's volume indicator reads it on every page. camTrackingModeFb stays always-on because Cameras.svelte (protected WIP) cannot receive onMount/onDestroy wiring. Bundle delta: +238 B total (teardown boilerplate offsets the removed flat subscriptions).*
...
The loop was resumed to address H4-followup, which was left unchecked (`- [ ]`) in the HIGH section. `initSignals()` now wires only 9 common subscriptions at boot; 27 per-page subscriptions are lazy-wired via `initDisplayRoutingSubscriptions()` and `initAudioMixerSubscriptions()` called from each page's `onMount`/`onDestroy`. An additional 5 dead `micCeiling3*` subscriptions were removed (no consumer exists). Per stop-condition #2, the loop will graduate after one MEDIUM item is committed in a subsequent iteration.
