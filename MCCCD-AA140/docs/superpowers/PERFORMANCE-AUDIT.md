...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 16 subscriptions moved out of initSignals() (routing: routingModeFb + autoRouteEnableFb → DisplayRouting onMount/onDestroy; mixer state: sceneRecallFb + audioLinkCeilings12Fb + ceiling mic mute/trim/lineOut/connected ×3 → AudioMixer onMount/onDestroy). camTrackingModeFb left always-on (Cameras.svelte is user WIP). Always-on subscriptions: ~36 → ~20. index.js +349 B (gating code in main chunk — expected).*
...
| 2026-05-04 | Loop final summary | f5201e1 | All HIGH (except H4-followup) addressed |
| 2026-05-25 | H4-followup per-page signal gating | ac27b2b | Always-on subs ~36 → ~20 (-16); index.js +349 B (expected) |
