...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 25 subscriptions removed from always-on `initSignals()`. 5 routing signals gated in `initRoutingSignals/teardownRoutingSignals` (wired DisplayRouting.svelte onMount/onDestroy). 20 AudioMixer state signals gated in `initAudioMixerSignals/teardownAudioMixerSignals` (wired AudioMixer.svelte onMount/onDestroy). camTrackingModeFb intentionally left always-on (Cameras.svelte protected). index-*.js +223 B for gating infra; AudioMixer-*.js -490 B (moved code to main chunk); net JS −267 B. initSignals() now wires only 9 subscriptions (down from 34).*
...
| 2026-06-11 | H4-followup per-page signal gating | pending iter-10 | 25 subscriptions removed from boot; initSignals() 34→9; index-*.js +223 B, AudioMixer-*.js -490 B, net JS −267 B |
