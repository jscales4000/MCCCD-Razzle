...
- [~] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DEFERRED iter-10** — *camTrackingModeFb gating requires editing Cameras.svelte (user WIP / hard-rule protected). Remaining signal splits (routing → DisplayRouting, mixer-state → AudioMixer) are medium-risk refactor for zero measurable CPU win — low-frequency signals fire only on user action, not continuously. Deferring; revisit if a future audit wants to prune crcomlib subscription count further and Cameras.svelte is no longer WIP.*
...
- [x] **M1. Move `unsubscribeSerial` / `publishSerial` exports out of `CrComLib.ts` (or annotate `/* @__PURE__ */`)** — *files: `src/lib/CrComLib.ts`* — *risk: low* — *expected: ~150 B JS (esbuild already tree-shakes, but verify)* — **DONE iter-10 (no bundle delta — tree-shaker already handled it)** — *Confirmed: `publishSerial`, `subscribeSerial`, `unsubscribeDigital`, `unsubscribeSerial` all absent from `dist/assets/index-*.js` before change; 4 dead exports deleted for source hygiene. Note: `unsubscribeAnalog` was listed in the original M1 item but is now used (added by H4 in iter-5) — kept. Bundle: 77.05 kB JS / 28.64 kB CSS (unchanged, as predicted).*
...
| 2026-05-28 | H4-followup DEFERRED + M1 dead exports | (iter-10) | H4-followup blocked by Cameras.svelte WIP; M1 4 dead CrComLib exports deleted — no bundle delta (tree-shaker already handled); 77.05 kB JS / 28.64 kB CSS (unchanged) |
