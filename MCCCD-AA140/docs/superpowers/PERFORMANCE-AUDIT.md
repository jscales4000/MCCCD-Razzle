...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 27 subscriptions moved out of initSignals() into initRoutingSignals() (7: D2/D3 source fb, D1-D3 power fb, routingMode fb, autoRoute fb — wired via DisplayRouting onMount/onDestroy) and initAudioMixerStateSubscriptions() (20: 5×trim, 5×lineOut, 3×ceilingMute, 5×connected, sceneRecall, audioLinkCeilings12 — wired via AudioMixer onMount/onDestroy). initSignals() now has 10 always-on subscriptions (down from 37). camTrackingModeFb stays always-on (Cameras.svelte is protected). Main bundle +642 B for tracking infrastructure; expected for a runtime-reduction refactor.*
