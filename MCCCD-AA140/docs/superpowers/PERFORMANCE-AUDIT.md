...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: exactly 25 subscriptions moved out of initSignals() into initAudioMixerStateSubscriptions() (20) + initRoutingStateSubscriptions() (5). index-*.js +179 B for gating scaffolding. camTrackingModeFb left always-on — Cameras.svelte is user WIP. progAudioLevelFb left always-on — Home's VolumePopup reads it. micLavMuteFb/micHandheldMuteFb left always-on — Home mic strip reads them.*
...
| 2026-06-12 | H4-followup per-page state signal gating | pending iter-10 | −25 subscriptions at idle; index-*.js +179 B for gating scaffolding |
