...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`, `src/pages/AudioMixer.svelte`* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial: 4 of the targeted signals)** — *actual: gated routingModeFb + autoRouteEnableFb (DisplayRouting-only) and sceneRecallFb + audioLinkCeilings12Fb (AudioMixer-only); camTrackingModeFb intentionally left in initSignals() since Cameras.svelte is user WIP and cannot be touched. Bundle JS +343 B (expected: added function bodies offset the 4 removed initSignals lines); runtime: 4 fewer crcomlib subscription entries during Home / non-routing / non-mixer views.*
