...
- [~] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DEFERRED iter-10** — *complete implementation requires wiring `camTrackingModeFb` teardown in `Cameras.svelte` (protected WIP file per hard rules). Partial impl (routing+mixer signals only) would leave camTrackingModeFb in common init, saving ~20 subscriptions for zero runtime CPU win — not worth the refactor complexity. Revisit after Cameras.svelte WIP is merged.*
...
- [x] **M1. Move `unsubscribeSerial` / `publishSerial` exports out of `CrComLib.ts` (or annotate `/* @__PURE__ */`)** — *files: `src/lib/CrComLib.ts`* — *risk: low* — *expected: ~150 B JS (esbuild already tree-shakes, but verify)* — **DONE iter-10** — *actual: 0 B bundle delta (esbuild was already tree-shaking all 4 dead exports: `publishSerial`, `subscribeSerial`, `unsubscribeSerial`, `unsubscribeDigital`). Dead code removed for hygiene. Note: `unsubscribeAnalog` was listed as unused in the original audit but IS now used in signals.ts line 153 (added by H4) — retained.*
...
| 2026-05-09 | H4-followup deferred; M1 dead-code removal | (this iter) | H4-followup: DEFERRED (Cameras.svelte WIP constraint); M1: 0 B delta (esbuild already tree-shook the 4 dead exports; removed for hygiene) |
