...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: boot-time subscription count 37 → 12 (-25). 20 AudioMixer-exclusive signals (ceiling mutes, all trims, all lineouts, all connected, sceneRecallFb, audioLinkCeilings12Fb) gated via `initAudioMixerStateSubscriptions()`. 5 DisplayRouting-exclusive signals (display1/2/3PowerFb, routingModeFb, autoRouteEnableFb) gated via `initDisplayRoutingSubscriptions()`. camTrackingModeFb kept in common init (Cameras.svelte is user WIP). index.js +349 B for gating boilerplate — expected, not a size-reduction item.*
...
| 2026-05-18 | H4-followup gate 25 state sigs | 1aa2331 | Boot subscriptions 37 → 12 (-25); index.js +349 B (gating overhead, expected) |
