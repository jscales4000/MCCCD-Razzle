...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial)** — *actual: 16 subscriptions removed from always-on `initSignals()` → gated to their consumer page. 2 routing signals (routingModeFb, autoRouteEnableFb) → `initDisplayRoutingSubscriptions()` wired in DisplayRouting.svelte onMount/onDestroy. 14 AudioMixer-state signals (sceneRecallFb, audioLinkCeilings12Fb, ceiling1-3 mute/trim/lineOut/connected) → `initAudioMixerStateSubscriptions()` wired in AudioMixer.svelte onMount/onDestroy. camTrackingModeFb left always-on (Cameras.svelte is user WIP). Bundle delta: ~neutral (code reorganized, not eliminated). Boot-time crcomlib registry: 16 fewer always-on callback refs.*
...
| 2026-07-01 | H4-followup gate low-freq signals | pending iter-10 | 16 subscriptions gated (2 routing → DisplayRouting, 14 mixer state → AudioMixer); ~neutral bundle; 16 fewer boot-time crcomlib refs |
