...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial)** — *actual: moved 16 subscriptions out of initSignals(): 14 AudioMixer-state (sceneRecallFb, audioLinkCeilings12Fb, ceiling mic mute/trim/lineOut/connected × 3) via initMixerStateSubscriptions + 2 DisplayRouting-state (routingModeFb, autoRouteEnableFb) via initRoutingSubscriptions. camTrackingModeFb intentionally left in initSignals() — Cameras.svelte is protected user WIP. index.js +523 B overhead for 4 new gating functions; runtime removes 16 idle crcomlib registrations.*
...
| 2026-05-10 | H4-followup per-page signal gating | TBD iter-10 | index.js +523 B overhead; 16 idle subscriptions removed from boot; camTrackingModeFb left (Cameras.svelte WIP) |
