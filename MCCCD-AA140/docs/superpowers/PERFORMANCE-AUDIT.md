...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 27 subscriptions moved out of initSignals (37→10 always-active). New initDisplayRoutingSignals (7 subs) and initAudioMixerStateSignals (16 subs) wired via onMount/onDestroy. 4 dead micCeiling3 subscriptions dropped (no consumer). camTrackingModeFb stays in initSignals — Cameras.svelte is WIP. Bundle neutral (JS -191 B index, noise).*
...
| 2026-06-08 | H4-followup per-page signal gating | 358c5c7 | initSignals 37→10 subs; 27 moved to DR/AM onMount; 4 dead Ceiling3 subs dropped; bundle neutral (JS -191 B) |
