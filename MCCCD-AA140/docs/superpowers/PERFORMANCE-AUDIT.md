...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`, `src/pages/AudioMixer.svelte`* — *risk: medium* — *expected: ~16 fewer subscriptions at idle; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (scoped, no Cameras.svelte)** — *actual: 16 subscriptions moved to per-page init/teardown (2 routing + 14 AudioMixer state). camTrackingModeFb left in initSignals() (Cameras.svelte protected). progAudioLevelFb left always-on (used by Aa140Footer on Home + DisplayRouting). Bundle index-*.js +348 B for the init/teardown scaffolding — expected trade-off for the runtime slot reduction. Runtime win: 16 fewer crcomlib callback registrations at idle (when user is on Home, Cameras, or navigating between).*
...
| 2026-06-24 | H4-followup per-page signal gating | pending iter-10 | 16 subscriptions moved from always-on to per-page mount/destroy; index-*.js +348 B for scaffolding; runtime: 16 fewer idle crcomlib slots |
