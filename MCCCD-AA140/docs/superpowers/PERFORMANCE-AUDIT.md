...
- [~] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DEFERRED (protected file)** — `camTrackingModeFb` subscription must move into `Cameras.svelte` (user WIP, DO NOT MODIFY). Without that, only a partial implementation is possible (DisplayRouting + AudioMixer only), and the audit item's own description calls for deferral unless the user requests further work. Revisit when `Cameras.svelte` WIP is merged.
...
- [x] **M1. Move `unsubscribeSerial` / `publishSerial` exports out of `CrComLib.ts` (or annotate `/* @__PURE__ */`)** — *files: `src/lib/CrComLib.ts`* — *risk: low* — *expected: ~150 B JS (esbuild already tree-shakes, but verify)* — **DONE iter-10** — *actual: JS +0 B (tree-shaker was already eliminating these 4 dead exports: `publishSerial`, `subscribeSerial`, `unsubscribeDigital`, `unsubscribeSerial`). Dead code removed regardless: shrinks public API surface and eliminates confusing "available but unused" functions.*
...
| 2026-05-13 | H4-followup DEFERRED | (doc only) | Requires Cameras.svelte (protected); deferred until WIP merged |
| 2026-05-13 | M1 drop 4 dead CrComLib exports | pending iter-10 | JS +0 B (tree-shaker already handled); API surface reduced by 4 functions |
