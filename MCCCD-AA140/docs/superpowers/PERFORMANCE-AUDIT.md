...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`, `src/pages/AudioMixer.svelte`* — *risk: medium* — *expected: ~23 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: removed 23 subscriptions from `initSignals()` (16 remain for cross-page/footer signals): 2 routing (routingModeFb, autoRouteEnableFb) → gated to DisplayRouting via `initRoutingSignals()`; 17 AudioMixer state signals (sceneRecallFb, audioLinkCeilings12Fb, audioOutputSelectFb, lav+handheld trim/lineOut/connected, ceiling1-2 mute/trim/lineOut/connected) → gated to AudioMixer via `initAudioMixerStateSignals()`; 4 dead ceiling-3 stores + subscriptions removed entirely (no UI consumer). camTrackingModeFb intentionally left in initSignals() — Cameras.svelte is user WIP. JS +90 B for gating logic (matches H4 scoped +253 B pattern); no CSS change.*
...
| 2026-06-15 | H4-followup per-page signal gating | pending iter-10 | 23 subs removed from initSignals() (16 remain); JS +90 B for gating logic; 4 dead ceiling-3 stores deleted |
