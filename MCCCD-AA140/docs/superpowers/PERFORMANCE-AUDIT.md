...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/AudioMixer.svelte`, `src/pages/DisplayRouting.svelte`* — *risk: medium* — *expected: ~21 fewer subscriptions at idle; minor memory; zero runtime CPU win* — **DONE iter-10 (partial)** — *actual: 16 analog + 9 digital = 25 subscription calls gated. `initAudioMixerSubscriptions()` covers 16 analogs + 9 digitals (was `initMicLevelSubscriptions` + 19 state signals). `initRoutingSubscriptions()` covers routingModeFb + autoRouteEnableFb. `camTrackingModeFb` stays always-on — Cameras.svelte is user-WIP. Lav/Handheld mutes stay always-on — shared with Home. Bundle: index JS +228 B (gate function overhead in signals.ts); routing/mixer chunks unchanged. Net: 25 fewer CrComLib callback registrations at idle (when not on AudioMixer or DisplayRouting).*
...
| 2026-05-04 | Loop final summary | f5201e1 | All HIGH addressed (at that time), loop summary written |
| 2026-05-19 | H4-followup per-page signal gating | pending iter-10 | index JS +228 B overhead; 25 subscriptions gated (21 AudioMixer-exclusive + 2 routing + 2 routing); camTrackingModeFb stays always-on (Cameras WIP) |
