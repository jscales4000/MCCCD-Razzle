...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial: Cameras.svelte protected)** — *actual: 25 subscriptions moved from always-on initSignals() to per-page lazy gates (20 → AudioMixer, 5 → DisplayRouting). initSignals() reduced from 37 → 12 subscriptions. index.js +179 B for the gating code; per-page runtime cost is the same or lower. camTrackingModeFb remains in initSignals() because Cameras.svelte is user WIP.*
...
| 2026-06-06 | H4-followup per-page signal gating | 99011c3 | initSignals 37→12 subs; −25 idle callbacks at Home/idle; index.js +179 B (gating boilerplate) |
