...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/AudioMixer.svelte`, `src/pages/DisplayRouting.svelte`* — *risk: medium* — *expected: ~16 fewer subscriptions at boot* — **DONE iter-10** — *actual: removed 14 ceiling-mic + mixer-state signals and 2 routing signals from `initSignals()` (always-on set drops from 37 → 21). Merged H4's `initMicLevelSubscriptions` into a combined `initAudioMixerSubscriptions`/`teardownAudioMixerSubscriptions` covering 19 signals total. `initDisplayRoutingSubscriptions`/`teardownDisplayRoutingSubscriptions` gates 2 signals. `camTrackingModeFb` left always-on (Cameras.svelte is user WIP). Main bundle +72 B (gating infrastructure); AudioMixer chunk -779 B (consolidated subscription calls).*
...
| 2026-06-13 | H4-followup per-page signal gating | pending iter-10 | always-on signals 37→21 (-16); AudioMixer chunk -779 B; main bundle +72 B |
