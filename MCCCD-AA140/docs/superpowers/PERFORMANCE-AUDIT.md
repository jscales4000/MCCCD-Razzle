...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 16 subscriptions removed from always-on initSignals() (2 routing + 14 AudioMixer ceiling/state). Added initRoutingSignals/teardownRoutingSignals wired into DisplayRouting.svelte onMount/onDestroy, and initAudioMixerStateSignals/teardownAudioMixerStateSignals wired into AudioMixer.svelte alongside the existing mic-level gate. index-*.js 76,699 B (compare: 77,221 B at iter-9, but note ~25 feature commits landed in between). camTrackingModeFb intentionally left always-on — Cameras.svelte is user WIP and cannot be modified.*
...
| 2026-05-04 | Loop final summary (iter-9) | f5201e1 | All HIGH addressed, stop conditions met — loop graduated |
| 2026-06-22 | H4-followup gate low-freq state signals | pending iter-10 | 16 subscriptions removed from always-on init (2 routing + 14 AudioMixer ceiling/state); index JS 76,699 B |
