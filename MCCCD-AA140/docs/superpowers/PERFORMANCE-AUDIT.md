...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 16 AudioMixer subscriptions (ceiling 1/2 mute/trim/lineOut/connected, sceneRecall, audioLinkCeilings12, lav/handheld trim/lineOut/connected) + 2 DisplayRouting subscriptions (routingModeFb, autoRouteEnableFb) moved to per-page lazy init/teardown via `initAudioMixerSubscriptions()` and `initRoutingSubscriptions()`. 4 dead ceiling-3 subscriptions (nobody reads those stores — Ceiling 3 strip dropped from AudioMixer) removed entirely. index JS 77,221 → 77,032 B (−189 B). camTrackingModeFb intentionally left always-on since Cameras.svelte is user WIP.*
...
| 2026-06-04 | H4-followup per-page state signal gating | pending iter-10 | 16 AudioMixer + 2 DisplayRouting subs lazy; 4 dead ceiling-3 subs removed; index JS −189 B |
