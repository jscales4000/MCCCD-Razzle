...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 16 subscriptions removed from always-on pool (39 → 23 at boot). `routingModeFb`+`autoRouteEnableFb` gated to DisplayRouting via `initRoutingSignals()`/`teardownRoutingSignals()`. `sceneRecallFb`+`audioLinkCeilings12Fb`+ceiling1/2 mute/trim/lineOut/connected (10 signals) gated to AudioMixer via `initMixerStateSignals()`/`teardownMixerStateSignals()`. 4 dead `micCeiling3*` subscriptions removed entirely (no consumer after 4-mic redesign). `camTrackingModeFb` left in initSignals() (Cameras.svelte protected). JS −745 B (removed inline closures exceed new function overhead); CSS +1,207 B from post-loop feature work (AppFooter, RoomPlan), not this change.*
...
| 2026-05-04 | Loop final summary (iter-9) | f5201e1 | All HIGH addressed, stop conditions met |
| 2026-06-27 | H4-followup gate low-freq state signals | pending iter-10 | 16 subs removed from always-on pool (39→23); JS −745 B; CSS +1,207 B from feature work |
