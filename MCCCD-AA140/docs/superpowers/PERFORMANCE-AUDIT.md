...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (scoped)** — *actual: 22 subscriptions removed from initSignals(): 2 routing signals gated to DisplayRouting (initRoutingSignals/teardownRoutingSignals), 16 AudioMixer-exclusive state signals gated to AudioMixer (initAudioMixerStateSignals/teardownAudioMixerStateSignals), 4 ceiling3 signals removed entirely (strip dropped in 4-mic redesign). camTrackingModeFb kept always-on since Cameras.svelte is protected WIP. JS -189 B (cosmetic); runtime: 22 fewer CrComLib registry entries at boot.*
...
| 2026-06-07 | H4-followup per-page state signal gating | pending iter-10 | 22 subscriptions removed from initSignals(); JS -189 B (cosmetic); 2 routing gated to DisplayRouting, 16 AudioMixer state gated to AudioMixer, 4 ceiling3 removed (dead) |
