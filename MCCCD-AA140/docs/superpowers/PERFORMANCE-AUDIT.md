...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 22 subscriptions gated (20 AudioMixer-only, 2 DisplayRouting-only). camTrackingModeFb, progAudioLevelFb, micLavMuteFb, micHandheldMuteFb stayed in initSignals() (shared or protected-file consumers). Bundle +452 B for new gate functions — expected for a runtime-optimization item. Cameras.svelte intentionally untouched (protected).*
...
| 2026-05-21 | H4-followup per-page signal gating | 98e31dd | 22 subscriptions gated; index JS +337 B / CSS +115 B (bundle grows for gate functions; runtime gains fewer crcomlib registry entries at idle) |
