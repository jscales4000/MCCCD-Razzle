...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (scoped)** — *actual: gated 20 AudioMixer-only state signals (ceiling mic mute/trim/lineOut/connected × 3, lav/handheld trim/lineOut/connected × 2, sceneRecallFb, audioLinkCeilings12Fb) + 2 DisplayRouting-only signals (routingModeFb, autoRouteEnableFb) = 22 subscriptions eliminated at boot. camTrackingModeFb stays always-on (Cameras.svelte protected). Bundle: index JS +589 B for new lazy-init stubs (expected — runtime tradeoff). Boot-time crcomlib registry reduced from ~37 to ~15 subscriptions.*
...
| 2026-05-08 | H4-followup per-page state signal gating | pending iter-10 | 22 boot-time subscriptions eliminated; index JS +589 B for stubs |
