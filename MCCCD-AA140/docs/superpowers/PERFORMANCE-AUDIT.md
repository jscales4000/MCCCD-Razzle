...
- [~] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DEFERRED iter-10** — *`camTrackingModeFb` must stay in `initSignals()` because `Cameras.svelte` is protected WIP; a partial implementation (DisplayRouting + AudioMixer signals only) is medium risk for zero CPU benefit. Per audit guidance "Defer unless user requests further perf work."*
...
- [x] **M1. Move `unsubscribeSerial` / `publishSerial` exports out of `CrComLib.ts` (or annotate `/* @__PURE__ */`)** — *files: `src/lib/CrComLib.ts`* — *risk: low* — *expected: ~150 B JS (esbuild already tree-shakes, but verify)* — **DONE iter-10** — *actual: JS -3 B (77,221 → 77,218); esbuild was already eliminating the 4 dead exports (publishSerial, subscribeSerial, unsubscribeSerial, unsubscribeDigital). Source is now cleaner. Note: `unsubscribeAnalog` was in the original item list but is actively used in `teardownMicLevelSubscriptions` (added in H4) — it was intentionally kept.*
...
| 2026-05-16 | H4-followup DEFERRED + M1 dead CrComLib exports | pending iter-10 | H4-followup deferred (Cameras.svelte protected); M1: JS -3 B (esbuild already tree-shaking, source cleaned) |
