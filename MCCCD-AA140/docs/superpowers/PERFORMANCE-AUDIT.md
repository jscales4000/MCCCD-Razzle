...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 27 subscriptions moved out of initSignals() (20 AudioMixer-only via initAudioMixerStateSubscriptions, 7 DisplayRouting-only via initRoutingSubscriptions). Common always-on pool shrunk from 37→10. index JS +349 B for gating function bodies in signals.ts (expected; win is runtime registry reduction). Cameras.svelte untouched — camTrackingModeFb stays common.*
...
| 2026-05-20 | H4-followup per-page signal gating | pending iter-10 | 27 subs moved to per-page onMount/onDestroy; initSignals 37→10 always-on; index JS +349 B (gating bodies); runtime: −27 crcomlib registry slots at idle |
