...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial)** — *actual: 16 subscriptions removed from boot (37 → 21 in initSignals). AudioMixer gates 14 (ceiling mic mute×3/trim×3/lineOut×3/connected×3 + sceneRecallFb + audioLinkCeilings12Fb); DisplayRouting gates 2 (routingModeFb + autoRouteEnableFb). camTrackingModeFb stays in initSignals — Cameras.svelte is protected user WIP. progAudioLevelFb stays — consumed by both Home and AudioMixer. index.js +349 B for gating logic (acceptable; not a size-reduction item).*
...
| 2026-05-06 | H4-followup gate AudioMixer+Routing subs | f3e18da | initSignals 37 → 21 subs (−16); index.js +349 B (gating code); not a size item |
