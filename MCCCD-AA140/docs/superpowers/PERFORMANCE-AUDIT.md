...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`, `src/pages/AudioMixer.svelte`* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 23 subscriptions moved from initSignals() to per-page gates. initSignals() now wires only 16 always-on subscriptions (down from 39). initRoutingSubscriptions() gates routingModeFb+autoRouteEnableFb (2) to DisplayRouting. initAudioMixerStateSubscriptions() gates sceneRecallFb, audioLinkCeilings12Fb, audioOutputSelectFb, mic trim×5, mic lineOut×5, mic connected×5, ceiling mute×3 (21) to AudioMixer. camTrackingModeFb intentionally stays always-on (Cameras.svelte is protected WIP). JS bundle +348 B main / +72 B lazy chunks — expected cost of teardown machinery.*
...
| 2026-06-17 | H4-followup per-page state signal gating | pending iter-10 | 23 subscriptions moved to per-page gates; initSignals() 39→16 always-on; index JS +348 B (teardown machinery cost) |
