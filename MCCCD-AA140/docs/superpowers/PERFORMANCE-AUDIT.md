...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`, `src/pages/AudioMixer.svelte`* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: 8 subscriptions removed from always-on initSignals(): 2 routing signals (routingModeFb, autoRouteEnableFb) now gated to DisplayRouting onMount/onDestroy; 2 scene/link signals (sceneRecallFb, audioLinkCeilings12Fb) now gated to AudioMixer onMount/onDestroy alongside H4's mic-level gating; 4 dead Ceiling3 non-level subscriptions removed entirely (Ceiling3 strip was dropped in the 5→4 mic redesign — no UI consumer remains). camTrackingModeFb stays always-on (Cameras.svelte is user WIP). index-*.js: 77,221 → 77,020 B (-201 B / -0.3%); runtime: 8 fewer subscription slots at boot.*
...
| 2026-05-30 | H4-followup gate routing+scene/link signals | pending iter-10 | 8 subscriptions removed from initSignals(); index-*.js 77,221 → 77,020 B (-201 B); 4 dead Ceiling3 subs removed |
