...
- [ ] **H3. Lazy-load Cameras / AudioMixer / DisplayRouting via `import()`** — *files: `src/App.svelte`* — *risk: medium* — *expected: 30–50% reduction in `index-*.js` first-paint cost; Home + Splash become the only synchronous bundle*
...
- [ ] **H4. Gate `signals.ts` initSignals() — only subscribe to signals the active page reads** — *files: `src/lib/stores/signals.ts`, `src/main.ts`* — *risk: medium* — *expected: ~30 fewer crcomlib subscriptions at boot; reduces signal callback storm during high-rate level updates (mic levels fire ~10–30 Hz on AudioMixer)*
...
- [ ] **H5. Inline-extract duplicate mic SVG icons in `Home.svelte` into a shared `<MicIcon>` component** — *files: `src/pages/Home.svelte` (read-only access OK on `MicVolumeModal.svelte`); new `src/lib/ui/MicIcon.svelte`* — *risk: low* — *expected: 0.3–0.5 KB gzipped CSS+HTML reduction; reduced DOM weight (~80 nodes saved per Home render)*
...
- [ ] **H6. Suppress dev-only Preview Dock + viewport listener in production builds** — *files: `src/pages/Home.svelte` (lines 126–143, 310–322)* — *risk: low* — *expected: ~0.8 KB JS off `index-*.js`; one fewer global `resize` listener at runtime*
...
- [ ] **H7. Convert `<svg>` `viewBox` constants in `Home.svelte` SVGs to a sprite or shared `<symbol>` set** — *files: `src/pages/Home.svelte`* — *risk: low* — *expected: ~1.5 KB raw HTML off `index-*.js` (Svelte template inlines this), and ~13 fewer SVG parse passes at first paint*
...
- [ ] **H8. Drop the dev-debug `window.onerror` overlay from production HTML** — *files: `build.mjs` (panel HTML template)* — *risk: low* — *expected: ~600 B HTML, but more importantly removes a permanent global error handler*
...
- [ ] **M5. Switch single-bundle CSS to `cssCodeSplit: true` with explicit chunks** — *files: `vite.config.ts`* — *risk: medium* — *expected: per-page CSS chunk loaded only when its page mounts; ~10 KB CSS off splash critical path*
...
- [ ] **M7. Add `passive: true` to the document-level click handler in `router.ts`** — *files: `src/lib/stores/router.ts` (line 344)* — *risk: low* — *expected: avoids a synchronous scroll-block on every click; primarily a hygiene win*
...
| 2026-05-03 | H2 drop glass-card backdrop-filter | pending iter-3 | CSS -27 B + runtime paint relief (panel-side) |
