...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial)** — *actual: 22 subscriptions removed from boot (`initSignals()` 38→15 calls); 2 routing signals (routingModeFb, autoRouteEnableFb) gated to DisplayRouting.svelte via `initRoutingSignals()`/`teardownRoutingSignals()`; 20 mixer-state signals (sceneRecallFb, audioLinkCeilings12Fb, 5×trim, 5×lineOut, 5×connected, 3×ceilingMute) gated to AudioMixer.svelte via `initAudioMixerStateSignals()`/`teardownAudioMixerStateSignals()`. Main bundle +409 B (teardown closure code can't tree-shake — signals.ts is main-chunk), offset by runtime registry win. camTrackingModeFb intentionally left in initSignals() — Cameras.svelte is user WIP and cannot be modified.*
...
| 2026-05-04 | Loop final summary | f5201e1 | All HIGH addressed, stop conditions met (loop ended) |
| 2026-05-26 | H4-followup per-page signal gating (partial) | pending iter-10 | initSignals() 38→15 subscriptions at boot; 22 gated (2 routing + 20 mixer-state); camTrackingModeFb stays (Cameras WIP); main bundle +409 B |
