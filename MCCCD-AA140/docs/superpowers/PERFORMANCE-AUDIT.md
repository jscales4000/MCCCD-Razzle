...
- [~] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DEFERRED iter-10 (protected file)** — *`camTrackingModeFb` gating requires `Cameras.svelte` (user WIP, hard-rule protected). Partial gating of DisplayRouting+AudioMixer signals without the Cameras teardown would leave the refactor inconsistent. Audit explicitly noted "Defer unless user requests further perf work" — zero runtime CPU win confirmed since these signals fire only on user action.*
...
| 2026-05-15 | H4-followup deferred (protected file) | iter-10 | camTrackingModeFb gating requires Cameras.svelte; zero CPU win confirmed; marked [~] |
| 2026-05-15 | M1 delete unused CrComLib exports | iter-10 | 4 exports removed (`publishSerial`, `subscribeSerial`, `unsubscribeDigital`, `unsubscribeSerial`); esbuild already tree-shook them (0 B delta); source cleanup only |
