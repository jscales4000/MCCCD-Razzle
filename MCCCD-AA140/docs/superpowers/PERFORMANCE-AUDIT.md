...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (scoped)** — *actual: 12 signals moved (2 to DisplayRouting, 10 to AudioMixer). camTrackingModeFb left in initSignals() (Cameras.svelte WIP). progAudioLevelFb left always-on (Aa140Footer shows volume on Home + DisplayRouting — discovered post-audit). micCeiling3* left in initSignals() (dead code since 4-mic hardware redesign, safe to remove in a future pass). crcomlib subscription count at idle: ~36 → ~24 always-on entries.*
...
| 2026-06-21 | H4-followup gate routing+ceiling state subscriptions per-page | 55aa8c6 | 12 signals moved from always-on to lazy; crcomlib boot subscriptions ~36 → ~24; index-*.js 76,750 B, index-*.css 29,730 B |
