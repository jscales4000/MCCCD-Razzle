...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (scoped, Cameras protected)** — *actual: boot subscriptions 39 → 9 (−30). Extracted `initMixerSubscriptions()`/`teardownMixerSubscriptions()` (21 AudioMixer-only subs: trims, lineOuts, ceiling mutes, connected, sceneRecall, audioLinkCeilings, audioOutputSelect) and `initRoutingSubscriptions()`/`teardownRoutingSubscriptions()` (9 DisplayRouting-only subs: display2-4 sourceFb, all 4 powerFb, routingMode, autoRoute). camTrackingModeFb stays in initSignals() — Cameras.svelte is user WIP. index JS −491 B (incidental, due to inlining differences); lazy chunks stable.*
...
| 2026-05-04 | Loop final summary (iter-9) | f5201e1 | All HIGH addressed (at the time), stop conditions met |
| 2026-06-25 | H4-followup per-page state signal gating | pending iter-10 | Boot subs 39 → 9 (−30); `initMixerSubscriptions()` (21 subs) + `initRoutingSubscriptions()` (9 subs); camTrackingModeFb deferred (Cameras WIP) |
