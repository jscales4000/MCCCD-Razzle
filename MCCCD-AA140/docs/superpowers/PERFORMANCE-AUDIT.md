...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, `src/pages/DisplayRouting.svelte`, `src/pages/AudioMixer.svelte`* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10** — *actual: initSignals() 36 → 16 subscriptions at boot; 5 routing signals (display1/2/3PowerFb, routingModeFb, autoRouteEnableFb) gated to DisplayRouting.svelte onMount; 16 mixer-state signals (sceneRecallFb, audioLinkCeilings12Fb, trim/lineOut/mute/connected for lav/handheld/ceiling1/2) gated to AudioMixer.svelte onMount. camTrackingModeFb stays always-on (Cameras.svelte is user-WIP). index.js +398 B for gating overhead (expected; item win is runtime subscriptions, not bundle size).*
...
| 2026-06-10 | H4-followup per-page state signals | pending iter-10 | initSignals 36→16 subs at boot; +398 B gating overhead (expected); 21 runtime subs deferred to page mount |
