...
- [x] **H4-followup. Per-page gate the remaining low-frequency state signals** — *files: `src/lib/stores/signals.ts`, all four page files* — *risk: medium* — *expected: ~25 fewer subscriptions at idle; minor memory; zero runtime CPU win since these don't fire continuously* — **DONE iter-10 (partial)** — *actual: 16 subscriptions removed from boot (14 AudioMixer state + 2 DisplayRouting). camTrackingModeFb left in initSignals() because Cameras.svelte is protected. Bundle: JS +181 B / CSS +115 B (boilerplate of 4 new init/teardown functions outweighs removed inline calls — win is runtime only).*
...
| 2026-05-04 | Loop final summary (iter-9) | f5201e1 | Loop declared done (prematurely — H4-followup was still unchecked) |
| 2026-05-29 | H4-followup gate low-freq state signals (partial) | pending iter-10 | 16 subscriptions removed from boot; JS +181 B / CSS +115 B (runtime win only) |
