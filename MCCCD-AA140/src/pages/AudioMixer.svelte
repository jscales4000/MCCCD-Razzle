...
<script lang="ts">
  ...
  // sceneRecallFb and audioLinkCeilings12Fb are also gated here (H4-followup).
  onMount(() => {
    initMicLevelSubscriptions();
    initAudioMixerStateSubscriptions();
  });
  onDestroy(() => {
    teardownMicLevelSubscriptions();
    teardownAudioMixerStateSubscriptions();
  });
