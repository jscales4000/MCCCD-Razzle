<!--
  DropZoneTile — drop-zone variant of a display tile.

  Used only by the experimental DragDropRouter sub-page (currentPage='dragdrop').
  The "regular" display tile with the per-display source-button grid is
  DisplayTile.svelte; this is its drag-drop counterpart, preserved while the
  drag-drop UX is parked pending chrome realignment to Mockup 10.

  Click handler routes when a chip is armed via the router store. The tile
  is identifiable to the drag system via class="tile" and data-display={id}.
-->
...
<script lang="ts">
  ...
  // Map analog feedback (1..4) → SourceId + label + inline SVG.
...
<style>
  ...
  /* DROP-HOVERING — solid accent + 1.02 scale; class added imperatively by router.ts. */
  ...
  /* tile-flash animation; class toggled by router.ts on drop. */
  :global(.tile.flash) {
