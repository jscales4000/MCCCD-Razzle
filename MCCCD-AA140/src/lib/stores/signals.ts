...
export function initSignals(): void {
  ...
  // micCeiling1-3MuteFb subscribed lazily — see initAudioMixerSubscriptions()

  // micLavTrimFb/LineOutFb, micHandheldTrimFb/LineOutFb, mic connected,
  // ceiling mic signals subscribed lazily — see initAudioMixerSubscriptions()

  // Mic real-time levels subscribed lazily — see initMicLevelSubscriptions()
  ...
  // routingModeFb / autoRouteEnableFb subscribed lazily — see initRoutingSubscriptions()

  subscribeAnalog(SIGNALS.progAudioLevelFb,    (v) => progAudioLevelFb.set(v));
  // sceneRecallFb / audioLinkCeilings12Fb subscribed lazily — see initAudioMixerSubscriptions()
}
...
// AudioMixer-specific state signals: trim, line-out, connected, ceiling mutes,
// scene recall, and link-ceilings toggle. These fire on user action (not
// continuously), but gating them drops 20 subscriptions from the boot registry.
...
// DisplayRouting-specific signals: routing mode + auto-route toggle.
