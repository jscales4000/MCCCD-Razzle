...
export function initSignals(): void {
  ...
  // ceiling mutes/trims/lineOuts/connected are AudioMixer-only → initAudioMixerStateSubscriptions()

  // Mic real-time levels are subscribed lazily → initMicLevelSubscriptions() (H4).
  // All other AudioMixer-only state signals → initAudioMixerStateSubscriptions() (H4-followup).
  ...
  // routing signals are DisplayRouting-only → initRoutingSubscriptions() (H4-followup)
...
// Low-frequency state signals consumed only by AudioMixer. Gated per-page
// to keep initSignals() lean (15 subs at boot vs 36 pre-followup).
...
// ── DisplayRouting subscriptions (H4-followup) ───────────────────────
// Routing mode and auto-route feedback are only read by DisplayRouting.
