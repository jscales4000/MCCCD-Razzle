...
export function initSignals(): void {
  ...
  // Ceiling mic mute/trim/lineOut/connected + lav/handheld trim/lineOut/connected
  // are gated to AudioMixer via initAudioMixerStateSignals().
  // Ceiling3 subscriptions removed — strip dropped in 4-mic redesign.
  ...
  // sceneRecallFb + audioLinkCeilings12Fb gated to AudioMixer via initAudioMixerStateSignals().
}
...
// Low-frequency state signals exclusive to AudioMixer. Subscribed only
// while AudioMixer is mounted; CrComLib re-fires current values on
// re-subscription so no stale-state risk.
...
// ── Routing signals (H4-followup) ─────────────────────────────────────
// DisplayRouting-exclusive signals. Subscribed only while DisplayRouting
// is mounted.
