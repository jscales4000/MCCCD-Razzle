...
export function initSignals(): void {
  ...
  // audioOutputSelectFb — lazy, AudioMixer only (see initMixerStateSubscriptions)
  ...
  // micCeiling mutes, lav/handheld/ceiling trims + lineOuts — lazy, AudioMixer only (see initMixerStateSubscriptions)
  ...
  // micLav/Handheld/Ceiling connected — lazy, AudioMixer only (see initMixerStateSubscriptions)
  ...
  // routingModeFb + autoRouteEnableFb — lazy, DisplayRouting only (see initRoutingStateSubscriptions)

  subscribeAnalog(SIGNALS.progAudioLevelFb,      (v) => progAudioLevelFb.set(v));
  // sceneRecallFb + audioLinkCeilings12Fb — lazy, AudioMixer only (see initMixerStateSubscriptions)
}
...
// ── AudioMixer page: state signals (not high-frequency, but mixer-only) ──────
// These fire only on user action; gating reduces the crcomlib registry footprint
// (~21 subscriptions) without any CPU cost impact.
...
// ── DisplayRouting page: routing mode + auto-route ────────────────────────────
