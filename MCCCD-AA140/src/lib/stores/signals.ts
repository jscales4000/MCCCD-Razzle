...
export function initSignals(): void {
  ...
  // Ceiling mic mutes, all mic trims/lineOuts/connected, sceneRecall,
  // audioLinkCeilings12, and real-time levels are gated to AudioMixer —
  // see initAudioMixerSubscriptions() below.

  // Routing signals are gated to DisplayRouting — see initRoutingSubscriptions().
  ...
  // camTrackingModeFb stays always-on: Cameras.svelte is user-WIP and cannot
  // be modified to wire its own onMount/onDestroy subscription lifecycle.
...
// Audit H4 + H4-followup: gate all AudioMixer-exclusive signals so they
// only live while AudioMixer is mounted. Includes:
//   • 5 real-time level analogs (10-30 Hz each — the original H4 reason)
//   • 19 low-frequency state signals exclusive to AudioMixer
// micLavMuteFb / micHandheldMuteFb stay in initSignals() — shared with Home.
...
export function teardownAudioMixerSubscriptions(): void {
  for (const id of audioMixerAnalogIds)  unsubscribeAnalog(id);
  for (const id of audioMixerDigitalIds) unsubscribeDigital(id);
  audioMixerAnalogIds  = [];
  audioMixerDigitalIds = [];
  // Reset VU meters so stale levels don't linger between visits.
  // State signals (mutes, trims, lineOut, connected) keep last value —
  // the panel will push fresh state immediately on re-subscribe.
  micLavLevel.set(0);
