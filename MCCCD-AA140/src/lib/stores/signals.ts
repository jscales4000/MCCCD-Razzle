...
export function initSignals(): void {
  ...
  // Display power, routing mode, AudioMixer channel state are also gated per-page
  // via initDisplayRoutingSubscriptions() / initAudioMixerSubscriptions() below.
...
// display1-4PowerFb, routingModeFb, autoRouteEnableFb are only consumed
// by DisplayRouting. Gate them so the crcomlib registry stays lean at idle.

let drAnalogIds: string[] = [];
let drDigitalIds: string[] = [];

export function initDisplayRoutingSubscriptions(): void {
  if (drAnalogIds.length + drDigitalIds.length > 0) return;
  drAnalogIds = [
...
// audioOutputSelectFb, ceiling/lav/handheld trim + lineOut + connected,
// sceneRecallFb, audioLinkCeilings12Fb — consumed only on the AudioMixer
// page. Gated alongside initMicLevelSubscriptions() for per-page hygiene.

let amAnalogIds: string[] = [];
let amDigitalIds: string[] = [];

export function initAudioMixerSubscriptions(): void {
  if (amAnalogIds.length + amDigitalIds.length > 0) return;
  amAnalogIds = [
