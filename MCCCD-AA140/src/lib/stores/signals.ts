...
// Wire the 10 signals consumed by Home (always-mounted) and shared state.
// Per-page signals are gated in initAudioMixerStateSubscriptions() and
// initRoutingSubscriptions() — called from each page's onMount/onDestroy.
...
// ── AudioMixer-only state subscriptions (H4-followup) ────────────────
// 20 low-frequency signals only consumed by AudioMixer: ceiling mutes,
// all 5-channel trims, line-outs, connected flags, scene recall, and the
// ceiling-link toggle. Fire on user action (not continuously), so gating
// saves ~20 crcomlib registry slots when the user is not on AudioMixer.
...
// ── DisplayRouting-only subscriptions (H4-followup) ──────────────────
// 7 signals only consumed by DisplayRouting: display 2+3 source feedback,
// all 3 display power flags, routing mode, and auto-route toggle. Gating
// saves ~7 crcomlib registry slots when the user is not on DisplayRouting.
