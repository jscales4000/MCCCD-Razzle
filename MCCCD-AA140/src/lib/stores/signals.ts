...
export function initSignals(): void {
  ...
  // Ceiling mic signals and routing/scene/link-ceilings signals are gated
  // per-page — see initAudioMixerStateSignals() and initRoutingSignals() below.
...
// ── AudioMixer state signals (low-frequency, gated to AudioMixer page) ───────
// Ceiling mic mute/trim/lineOut/connected + scene/link signals are only
// consumed by AudioMixer. Gate them so they don't hold references at idle.
...
// ── DisplayRouting signals (low-frequency, gated to DisplayRouting page) ─────
