...
export function initSignals(): void {
  ...
  // micLavMuteFb / micHandheldMuteFb stay always-on: Aa140Footer reads them on
  // Home and DisplayRouting. Ceiling mute/trim/lineOut/connected are AudioMixer-only
  // and moved to initAudioMixerStateSubscriptions() below.
  ...
  // camTrackingModeFb stays always-on: Cameras.svelte (protected user WIP) reads it.
  subscribeAnalog(SIGNALS.camTrackingModeFb,   (v) => camTrackingModeFb.set(v === 2 ? 2 : v === 3 ? 3 : 1));

  // progAudioLevelFb stays always-on: Aa140Footer shows it on Home and DisplayRouting.
...
// Audit H4-followup: routingModeFb and autoRouteEnableFb are only read by
// DisplayRouting. Gate them so the stores are live only while that page is
// mounted. No reset on teardown — the processor pushes current state on
// re-subscribe.
...
export function teardownRoutingSubscriptions(): void {
  routingAnalogIds.forEach((id) => { if (id) unsubscribeAnalog(id); });
  routingDigitalIds.forEach((id) => { if (id) unsubscribeDigital(id); });
  routingAnalogIds = [];
  routingDigitalIds = [];
}

// ── AudioMixer state subscriptions (low-frequency) ───────────────────────────
// Audit H4-followup: these state signals fire only on user action (scene
// recall, trim change, output select, etc.) and are exclusively consumed by
// AudioMixer. Gate them per-page. Ceiling-3 signals are included even though
// AudioMixer currently only imports ceiling 1-2, so they don't waste registry
// slots at idle.
...
export function teardownAudioMixerStateSubscriptions(): void {
  mixerStateAnalogIds.forEach((id) => { if (id) unsubscribeAnalog(id); });
  mixerStateDigitalIds.forEach((id) => { if (id) unsubscribeDigital(id); });
  mixerStateAnalogIds = [];
  mixerStateDigitalIds = [];
}
