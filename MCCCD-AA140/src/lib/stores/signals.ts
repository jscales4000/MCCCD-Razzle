...
export function initSignals(): void {
  ...
  // Per-page lazy subscriptions (see below):
  // Mic levels          → initMicLevelSubscriptions()       AudioMixer onMount
  // AudioMixer state    → initAudioMixerStateSubscriptions() AudioMixer onMount
  // DisplayRouting sigs → initDisplayRoutingSubscriptions()  DisplayRouting onMount
...
// Audit H4-followup: 20 state signals only consumed by AudioMixer. Gated
// per-page to trim boot-time subscription count from 37 → 12.

let audioMixerDigitalIds: string[] = [];
let audioMixerAnalogIds: string[] = [];

export function initAudioMixerStateSubscriptions(): void {
  if (audioMixerDigitalIds.length > 0) return; // idempotent
...
// Audit H4-followup: 5 signals only consumed by DisplayRouting.
...
export function initDisplayRoutingSubscriptions(): void {
  if (displayRoutingDigitalIds.length > 0) return; // idempotent
