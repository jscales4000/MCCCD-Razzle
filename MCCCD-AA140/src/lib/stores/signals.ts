...
// Gated per H4-followup to reduce the always-on crcomlib subscription count.

let routingSubCleanups: Array<() => void> = [];

export function initRoutingSubscriptions(): void {
  if (routingSubCleanups.length > 0) return;
  const rid1 = subscribeAnalog(SIGNALS.routingModeFb,      (v) => routingModeFb.set(v));
  const rid2 = subscribeDigital(SIGNALS.autoRouteEnableFb, (v) => autoRouteEnableFb.set(v));
  routingSubCleanups = [
    () => { if (rid1) unsubscribeAnalog(rid1); },
    () => { if (rid2) unsubscribeDigital(rid2); },
  ];
}

export function teardownRoutingSubscriptions(): void {
  routingSubCleanups.forEach(fn => fn());
  routingSubCleanups = [];
}

// ── AudioMixer state lazy subscriptions ──────────────────────────────
// Ceiling mic config + scene/link signals are only consumed by AudioMixer.
// Subscribed on-mount alongside initMicLevelSubscriptions() (H4), torn down
// on-destroy, so they're absent from the crcomlib registry when not on that page.

let mixerStateSubCleanups: Array<() => void> = [];

export function initAudioMixerStateSubscriptions(): void {
  if (mixerStateSubCleanups.length > 0) return;
  const aids = [
  ...
  mixerStateSubCleanups = [
    ...aids.map(id => () => { if (id) unsubscribeAnalog(id); }),
    ...dids.map(id => () => { if (id) unsubscribeDigital(id); }),
  ];
}

export function teardownAudioMixerStateSubscriptions(): void {
  mixerStateSubCleanups.forEach(fn => fn());
  mixerStateSubCleanups = [];
}
