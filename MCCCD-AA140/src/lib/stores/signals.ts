...
// Wire always-needed feedback subscriptions on app startup. Called from
// src/main.ts after CrComLib is detected.
//
// Per-page lazy subscriptions handle the rest:
//   initMicLevelSubscriptions / teardownMicLevelSubscriptions → AudioMixer (5 high-freq level signals)
//   initMixerStateSubscriptions / teardownMixerStateSubscriptions → AudioMixer (14 state signals)
//   initRoutingSubscriptions / teardownRoutingSubscriptions → DisplayRouting (2 signals)
//   camTrackingModeFb stays here: Cameras.svelte is user WIP, cannot add onMount/onDestroy.
export function initSignals(): void {
  ...
  // camTrackingModeFb: only Cameras.svelte consumes this; ideally gated per-page
  // (audit H4-followup) but Cameras.svelte is user WIP — cannot add onMount/onDestroy.
...
export function teardownMicLevelSubscriptions(): void {
  ...
  micLevelSubscriptionIds = [];
  // Reset stored values so a stale meter level doesn't linger if the user
...
// Audit H4-followup: ceiling mic mute/trim/lineOut/connected + scene/link signals
// are only consumed by AudioMixer. Gate them per-page to shed 14 idle subscriptions
// while the user is on Home, Cameras, or DisplayRouting.
